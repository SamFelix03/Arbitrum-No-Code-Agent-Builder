...
/**
 * Fetch ETH balance (native token of Arbitrum Sepolia testnet)
 * @param address - The wallet address
 * @returns ETH token balance as string
 */
export async function getTokenBalances(address: string): Promise<{
  ...
  try {
    ...
    // Get native ETH balance (native token, not ERC-20)
    const balance = await provider.getBalance(address)

    // Format balance (ETH uses 18 decimals)
